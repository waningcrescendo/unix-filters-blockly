...
Blockly.constants.Sort = {}
Blockly.constants.Sort.SORT_MUTATOR_MIXIN = {
  optionCount_: 0,
  ...
  decompose: function (workspace) {
    const containerBlock = workspace.newBlock('sort_mutator_container')
    containerBlock.initSvg()
    let connection = containerBlock.getInput('OPTIONS').connection
    for (let i = 0; i < this.optionCount_; i++) {
      const optionBlock = workspace.newBlock('sort_mutator_option')
      optionBlock.initSvg()
...
export default Blockly.constants.Sort.SORT_MUTATOR_MIXIN
